...
describe('MongoClientFactory', () => {
  ...
  beforeEach(async () => {
    client = await factory.createClient('test', { url: 'mongodb://localhost:27017/interviewTracker-backend-test1' });
  });
  ...
  it('creates a new client if it does not exist a client with the given name', async () => {
    const newClient = await factory.createClient('test2', { url: 'mongodb://localhost:27017/interviewTracker-backend-test2' });
  ...
  it('returns a client if it already exists', async () => {
    const newClient = await factory.createClient('test', { url: 'mongodb://localhost:27017/interviewTracker-backend-test3' });
